...
void FormPodtNorm::paste()
{
    ...
    foreach (mnorm m, buf) {
        QSqlQuery query;
        query.prepare("insert into wire_podt_norm (id_podt_type, id_type_src, id_provol, id_buht_diam, id_src_diam, id_diam, id_matr, kvo, id_vid, dat_beg, dat_end) values "
                      "(:id_podt_type, :id_type_src, :id_provol, :id_buht_diam, :id_src_diam, :id_diam, :id_matr, :kvo, :id_vid, :dat_beg, :dat_end)");
...
{
    QSqlQuery query;
    query.prepare("select p.id_type, pp.id_pr, pp.id_dim, coalesce(wps.id_diam,pp.id_dim) as id_fromdim, p.id_diam, coalesce(wps.id_type,0) as id_type_src,  wpt.snam, coalesce(wpt2.snam ,'-'),  pr.nam, "
                  ...
                  "group by p.id_type, pp.id_pr, pp.id_dim, p.id_diam, wpt.snam, pr.nam, d.sdim, dp.sdim, wps.id_diam, ds.sdim, wps.id_type, wpt2.snam "
    ...
    if (query.exec()){
        setQuery(query);
        setHeaderData(6,Qt::Horizontal,QString::fromUtf8("Тип полуфабриката"));
        setHeaderData(7,Qt::Horizontal,QString::fromUtf8("Исход. полуфабрикат"));
        setHeaderData(8,Qt::Horizontal,QString::fromUtf8("Марка"));
        setHeaderData(9,Qt::Horizontal,QString::fromUtf8("Ф кат."));
        setHeaderData(10,Qt::Horizontal,QString::fromUtf8("Ф нач."));
        setHeaderData(11,Qt::Horizontal,QString::fromUtf8("Ф кон."));
...
{
    QSqlQuery query;
    query.prepare("select distinct id_podt_type||':'||id_provol||':'||id_buht_diam||':'|| id_src_diam||':'|| id_diam || ':' || id_type_src as nam from wire_podt_norm order by nam");
    if (query.exec()){
        exList.clear();
        while(query.next()){
