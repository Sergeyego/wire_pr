...
bool QrEncode::encodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, char* lpsSource, int ncSource)
{
	int i, j;
    ...
    int ncT=(ncDataCodeWord * 8) - m_ncDataCodeWordBit;
    int ncTerminater = (ncT<=4) ? ncT : 4;
...
bool QrEncode::data(int i, int j)
{
    return m_byModuleData[i][j];
}
...
int QrEncode::version()
{
    return m_nVersion;
}

bool QrEncode::autoExtent()
{
    return m_bAutoExtent;
}

int QrEncode::maskingNo()
{
    return m_nMaskingNo;
}

int QrEncode::GetEncodeVersion(int nVersion, char* lpsSource, int ncLength)
{
...
bool QrEncode::EncodeSourceData(char* lpsSource, int ncLength, int nVerGroup)
{
...
int QrEncode::GetBitLength(unsigned char nMode, int ncData, int nVerGroup)
{
	int ncBits = 0;
...
int QrEncode::SetBitStream(int nIndex, unsigned short wData, int ncData)
{
	int i;
...
bool QrEncode::IsNumeralData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return true;
...
bool QrEncode::IsAlphabetData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return true;
...
bool QrEncode::IsKanjiData(unsigned char c1, unsigned char c2)
{
...
unsigned char QrEncode::AlphabetToBinaly(unsigned char c)
{
...
unsigned short QrEncode::KanjiToBinaly(unsigned short wc)
{
...
void QrEncode::GetRSCodeWord(unsigned char * lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	int i, j;
...
void QrEncode::SetFunctionModule()
{
	int i, j;
...
void QrEncode::SetCodeWordPattern()
{
	int x = m_nSymbleSize;
...
void QrEncode::SetMaskingPattern(int nPatternNo)
{
	int i, j;
...
void QrEncode::SetFormatInfoPattern(int nPatternNo)
{
	int nFormatInfo;
	int i;
